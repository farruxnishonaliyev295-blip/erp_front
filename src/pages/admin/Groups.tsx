...
import EntityCard, {
  type StatusOption as CardStatusOption,
...
const formatSchedule = (schedules?: GroupDetails["schedules"]) => {
  ...
  const days = schedules
    .map((s) => WEEKDAY_UZ[s.weekDay] || s.weekDay)
    .join(", ");
...
const getRoomName = (schedules?: GroupDetails["schedules"]) => {
  ...
  return (
    schedules.find((s) => s.room?.name)?.room?.name ||
    "Biriktirilmagan"
  );
};

const getTeacherName = (
  groupTeachers?: GroupDetails["groupTeachers"],
) => {
  if (!groupTeachers?.length) {
    return "Biriktirilmagan";
  }

  const user = groupTeachers[0]?.teacher?.user;
...
export default function Groups() {
  ...
  const [viewGroupId, setViewGroupId] =
    useState<number | null>(null);

  const [viewGroup, setViewGroup] =
    useState<GroupDetails | null>(null);
  ...
  const fetchGroups = () => {
    ...
    groupsApi
      ...
      .then((response) => {
        setGroups(response.items as GroupRow[]);
  ...
  const addGroup = async (
    ...
    try {
      ...
      if (!weekDays.length) {
        throw new Error(
          "Kamida bitta dars kunini tanlang",
        );
      }
  ...
  return (
    <div>
      ...
      <p className="mt-1 mb-4 text-sm text-muted-foreground">
        {statusCounts.ACTIVE ?? 0} aktiv ·{" "}
        {statusCounts.PLANNED ?? 0} rejalashtirilgan ·{" "}
        {statusCounts.COMPLETED ?? 0} yakunlangan
      </p>
      ...
      {editLoading && (
        <p className="mb-4 text-sm text-muted-foreground">
          Tahrirlash uchun ma'lumot yuklanmoqda...
        </p>
      )}
      ...
      <EntityTable
        ...
        deleteDescription={() =>
          "Ushbu guruhni o'chirishni tasdiqlaysizmi? Barcha bog'liq ma'lumotlar o'chiriladi."
        }
        columns={[
          {
            key: "name",
            ...
            render: (group) => (
              ...
              >
                <span className="erp-person-avatar erp-person-initials">
                  {group.name
                    .slice(0, 2)
                    .toUpperCase()}
                </span>
                ...
                {[
...

                  {
                    ...
                    value:
                      viewGroup.course
                        ?.durationMonth
                        ? `${viewGroup.course.durationMonth} oy`
                        : "—",
                  },
...

                  {
                    label:
                      "O'quvchilar",
                    value: `${viewGroup.studentGroups?.length || 0}/${viewGroup.maxStudent}`,
                  },
                ].map((row) => (
                  <div
                    key={row.label}
